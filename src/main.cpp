...
// Ne marche pas (pas trouvé la solution)
...
void CheckBtn(Piece& piece,Piece& nextPiece, Grid& grid, bool& test){
    ...
    static int moveCooldown = 150;              // Temps d'attente entre deux mouvements (en ms), modifier pour des mouvements plus rapides
    ...
    if (moveClock.getElapsedTime().asMilliseconds() >= moveCooldown) {
        ...
        if (input.getButton().down == true) {
            test = grid.movePiece(piece, 0, 1); // Déplacement vers le bas
            moveClock.restart();
        }
    }
    ...
    if(input.getButton().r == true && gameOver){ // Restart le jeu
        score = 0;
        Level = 1;
        nbTot = 0;
        grid.empty();
    ...
    if(input.getButton().up == true && !spacePressedPreviously){
        ...
        rotateSound.play(); // Jouez le son de rotation (ne marche pas)
    }
